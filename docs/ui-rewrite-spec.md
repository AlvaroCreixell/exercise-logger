...
- **Sprint 5 shipped (2026-04-17):** `TodayScreen` hero-card redesign with in-card Start CTA; warm accent color (`--accent-warm`) introduced paired with CTA purple; training-cadence ribbon on `LastSessionCard` + eyebrow on `TodayScreen` (both fire at `sessionsLast7Days >= 3`); bottom-nav pill-filled active state + press feedback; day-switch and tab-nav `fade-in-soft` transitions; adopted `SectionHeader` at 5 callsites (also backfilled the missing Sprint 4 unit test); adopted `EmptyState` at 3 callsites (Today no-routine, History empty, ExerciseHistory empty) — EmptyState gained a `headingLevel` prop to support nested usage; adopted `Stat` in `SessionProgress`, `DayPreview`, `LastSessionCard`; `SetLogSheet` prefill refactored to open-edge detection via `useRef` (closes the 13-dep clobber bug with 3 regression tests); added `prefers-reduced-motion` respect to all `fade-in-soft` / `flash-logged` / `save-pulse` keyframes; `computeTrainingCadence` service + `useTrainingCadence` hook landed (UTC-midnight calendar math). New integration tests: `TodayScreen.test.tsx` (5 tests covering all three states), `WorkoutScreen.test.tsx` smoke (5 tests covering empty-state, active-session render, SessionProgress, SetLogSheet open, finish flow). Test count: 503 → 530.
